  ...
  if (length(selected.transcripts) > minimum.n.transcripts.biomarkerpanel){
    ...
    if (length(selected.transcripts) > minimum.n.transcripts.biomarkerpanel){
      ...
      if (nlevels(dgeTraining$samples$group) == 2){
        ...
        print(confusion.matrix.evaluated$diag)
        return(AUC)
      } 
    } else {
      AUC <- Inf
...
thrombo.algo.classify.evaluation.set <- function(dge = dge, 
  ...
  real.groups.training <- droplevels(dge$samples[training.samples, "group"])
  real.groups.evaluation <- droplevels(dge$samples[evaluation.samples, "group"])
...
thrombo.algo.classify.validation.set <- function(dge.tool = dge, 
  ...
  real.groups.training <- droplevels(dge$samples[training.samples, "group"])
  real.groups.validation <- droplevels(dge$samples[validation.samples, "group"])
